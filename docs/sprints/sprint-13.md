...
|---|-------|------|--------|-------|
| 15.1 | Spike : stack RAG (modèle d'embedding local, format d'index, perf/qualité ARM) → **ADR avant 15.2** | M | ✅ Done | Corpus test ~10²-10³ notes FR. Tranche par la mesure : modèle d'embedding (tag `-q4_0` si dispo), chunking, format de stockage, stratégie incrémentale. Lister ce que le spike NE couvre PAS (leçon S10/S11). |
| 15.2 | Index d'embeddings incrémental (`%APPDATA%\Doku\rag`, hash, borné ARM, 0 réseau) | L | ✅ Done | **Risque n°1 du cap.** Ajout/modif/suppression → ré-index incrémental ; n'empêche jamais l'usage de l'app ; écritures atomiques (pattern snapshots 7.2). |
| 15.3 | Q&A dossier avec citations sources + mode « document courant » (gros docs) | M | ✅ Done | Top-k → réponse citant les notes (nom cliquable → ouvre le fichier, pattern 9.4). Solde la lecture partielle de 14.3 (badge) pour les documents > fenêtre. |
...
- **15.3 codée (validation native en attente)** : Q&A « dossier » (portée sélectionnable dans la face Contexte — le câblage des puces `+ Contexte` de 14.0) → passages top-k de l'index 15.2 + réponse streamée + pied « **Passages consultés** » **déterministe** (les notes réellement fournies au modèle, noms cliquables → ouvre le fichier ; supprimé sur refus — solde d'avance le STOP/GO citations) ; mode « **document courant** » : doc > 12 000 car. interrogé EN ENTIER via un index éphémère (cache par hash), badge 14.3 → « Document entier (index) », **phrase de refus dédiée** (« extraits consultés » — jamais d'affirmation d'absence sur tout le doc, FR-4) ; repli 14.3 intact (OpenAI ou sans modèle d'embed). Garde-fous : premier index complet JAMAIS lancé depuis une question (garde validant modèle+version du meta), Stop = annulation propre (retire aussi le refresh inline), messages d'info hors historique. Critic 3 HIGH + reviewer 2 Majors intégrés. 248 tests, svelte-check, build, bascule de portée vérifiée en navigateur. À vérifier natif : question dossier citée (clic source → ouvre), refus honnête, gros doc (info après le 12 000ᵉ car. trouvée), cohabitation RAM embed+chat, 0 réseau.
- **15.3 validée par l'utilisateur (natif)** — question dossier citée, refus honnête, gros doc couvert en entier, RAM OK, 0 réseau. **Ledger 60/60 : sprint 3/3, checkpoint 100 % franchi, clos J+4 (comme S12).** Le cap v2.1 (« le copilote comprend le dossier entier ») est livré. Reste : rétro.
