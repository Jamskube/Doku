...
| 3.2 | Sauvegarde atomique + indicateur « modifié » (Ctrl+S) | S | P0 | ✅ Done | `saveActive` durci (marque « enregistré » seulement après écriture réussie) ; tmp+rename validé en natif par l'utilisateur |
...
| 2.2 | Nouvel onglet actif + différenciateur d'homonymes | S | P0 | ✅ Done | `tabs.ts` (`parentFolder`/`tabDiscriminator`, 7 tests) ; suffixe dossier parent dans l'onglet quand homonyme |
...
- **2.2** ✅ **Done** : module pur `tabs.ts` (`parentFolder` + `tabDiscriminator`, 7 tests) ; l'onglet affiche le dossier parent en suffixe discret **uniquement** si un autre onglet ouvert porte le même nom (FR-4). Le parent rétrécit avant le nom quand l'espace manque. Vérifié Playwright (2× `notes.md` → « Notes »/« Archives »).
- **Sprint 1 implémenté** : 5/5 stories codées. Ledger : 4/5 `passes:true` (1.1, 2.2, 3.3, 3.4). **3.2** reste en Review — code vérifié en navigateur, en attente du smoke test natif (tmp+rename sur vrai fichier) côté utilisateur.
- **3.2 validé** ✅ : tmp+rename confirmé en natif par l'utilisateur (Ctrl+S sur un vrai fichier). **Sprint 1 complet : 5/5 `passes:true`.** Prêt pour `/sprint retro`.
