...
- **10.4 — Export DOCX (stretch, ADR-0010)** : walker `marked.lexer` → modèle `docx` (headings/emphase/listes/liens/**tableaux**/code ; tout token non mappable → texte brut, **jamais de crash**). `docx.ts` (`mdToDocx`/`plainDocx`/`exportDocx` DI) + `tauri.ts` `saveDocxDialog` (dialog + `writeFile` **binaire**) ; capability **+`fs:allow-write-file`** ; 3e bouton doc-head. **`docx` (~100 Ko) lazy-loadée** par import dynamique au clic → hors bundle principal ; **`Packer.toBlob`** (pas `toBuffer`→Buffer Node). 7 tests (validité zip OOXML `PK`, non-crash, fallback). Listes ordonnées = préfixe `1.` (pas de numbering natif), images → alt. **10.4 `passes:false`** : attend l'essai NATIF (`.docx` ouvert dans Word/LibreOffice).
