...
### 2026-07-09 — 1.5
- **1.5** → Review : widget image dans le live-preview (`Image` → `<img>` sur ligne inactive, raw sur ligne active). Résolution `images.ts` (`isExternalUrl`, `resolveLocalImagePath` — 6 tests) ; `imageSrc` = externe tel quel / local résolu au dossier (facet `docDirFacet` fourni par DocumentView) + `convertFileSrc` (natif). Placeholder discret si erreur de chargement (**conteneur stable muté**, pas de `replaceWith` — sinon clobbered par CM6). `assetProtocol` activé (tauri.conf). Validé Playwright : image manquante → placeholder « Image introuvable — {alt} », sans casser la mise en page. Reste : afficher une **vraie** image locale en natif (bloqué en navigateur).
