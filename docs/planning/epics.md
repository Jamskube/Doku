...
## Epic 17 : Backend d'inférence NPU (perf) — ❌ **CLOS : NO-GO mesuré** (sprint 15, 2026-07-24)
...
**État** : ❌ **CLOS le 2026-07-24 sur un NO-GO chiffré** ([ADR-0016](../adr/0016-backend-inference-npu-foundry-local.md) `rejected`, spike 17.1). Le NPU **fait bien tourner un LLM** — mais pas via Foundry Local (son EP QNN est cassé, bug amont), et **aucun modèle QNN ne convient sur 16 Go** : le 1,5B tient mais est trop bête, le 7B est bon mais swappe, et le 3B qui les concilierait **n'existe pas**. Prefill mesuré 4-6× plus rapide, decode 2× plus lent → pas de point de fonctionnement. Le copilote **reste sur Ollama CPU `qwen2.5:1.5b-instruct-q4_0`**. **Réouverture** conditionnée aux 3 critères de l'ADR-0016 (3B QNN FR publié / machine ≥ 32 Go / runtime prefill-NPU+decode-CPU).
...
> **Décomposition (sprint 15)** : **spike-first, avec gate STOP/GO.** Le préalable « épuiser le levier CPU + Q4_0 + vrai 4B » a été **sauté par choix produit** (2026-07-24). 17.2 (la réécriture lourde) n'était **codée que si 17.1 tranchait GO**.
>
> **➜ Issue : 17.1 a tranché NO-GO. La gate ne s'est jamais ouverte, 17.2 est annulée sans avoir été codée** — c'est le mécanisme qui a fonctionné, pas un échec. Le repli prévu (basculer sur `qwen2.5:3b-instruct-q4_0`) a lui aussi été **écarté par l'utilisateur** : trop lent, trop de RAM pour un copilote qui doit rester un **gadget discret**. Statu quo assumé sur le **1.5b**. Détail chiffré : [ADR-0016](../adr/0016-backend-inference-npu-foundry-local.md), banc conservé dans `spike/npu-17.1/`.

### Stories
| # | Title | Size | Priority | Acceptance |
|---|---|---|---|---|
| 17.1 | ✅ **FAITE → NO-GO.** Spike : Foundry Local sur ce Snapdragon X Plus (mesure prefill + 0-réseau) | M | P1 | Given `foundry`/ONNX Runtime + EP **QNN** empaqueté en sidecar, when je le lance sur ce SoC et génère sur un **long doc réel**, then le **prefill est mesuré** (NPU vs CPU q4_0, chiffres relevés — le ~18× espéré est confirmé ou réfuté) ; **0 requête non-`localhost`** prouvée (monitor réseau pendant l'inférence, y compris activation du modèle) ; faisabilité d'un **client OpenAI** (`/v1/chat/completions`) confirmée ; un **ADR-0016** documente le verdict **GO/NO-GO tranché par la donnée** — **avant** tout code de production. STOP si le gain prefill ne vaut pas la complexité (→ NO-GO, repli CPU documenté) |
| 17.2 | ❌ **ANNULÉE** (gate jamais ouverte). Abstraction couche d'inférence (Ollama ⇄ Foundry) + client OpenAI — **gated 17.1=GO** | L | P1 | **Déverrouillée seulement si 17.1 = GO.** Given l'inférence abstraite derrière une interface, when je bascule Ollama ⇄ **Foundry Local**, then le copilote (chat/Q&A/résumé) fonctionne via Foundry avec **client OpenAI** (`/v1/chat/completions`, streaming, **annulation < 500 ms**) ; **0 réseau re-prouvé** ; gestion des modèles ONNX ; **re-packaging ARM64** vérifié ; bascule de retour vers Ollama sans régression |
