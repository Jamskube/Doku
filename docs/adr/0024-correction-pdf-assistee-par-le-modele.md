...
4. **La typographie est alignée sur celle du document**, pas sur celle du modèle : apostrophes, guillemets et tirets sont ramenés aux caractères que la ligne d'origine emploie réellement — la seule preuve locale que la police sait les écrire. **On ne déplie jamais `œ` en `oe` ni ne désaccentue une majuscule** : ce serait réintroduire une faute pour en faire passer une autre. Ces cas-là se refusent en clair.
