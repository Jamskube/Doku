...
Depuis `spike/`, lancer `npm install`, puis `npm run dev -- --port 1421 --host 127.0.0.1`, et ouvrir `/pdf-text-selection.html`. Le paramètre `?backingScale=1`, `?backingScale=1.25` ou `?backingScale=1.5` force uniquement le backing store du canvas ; le résultat expose séparément `backingScale`, le vrai `devicePixelRatio` et `backingScaleMatchesDevicePixelRatio`.

Le kill-test automatique exige, pour chaque page du corpus synthétique tournée à 0°, 90°, 180° et 270°, une sélection textuelle non vide, au moins un rectangle, des coordonnées entièrement contenues dans la page, un écart CSS canvas/TextLayer/page inférieur ou égal à 2 px et un backing store conforme à l’échelle demandée. La matrice `backingScale` 1/1,25/1,5 vérifie donc le canvas et les rotations, pas plusieurs DPR réels de TextLayer. Le smoke natif Tauri/WebView2 passe à 100 % (`devicePixelRatio = 1`), y compris une sélection multiligne réelle sur la page tournée à 270°. Les mises à l’échelle WebView2 125 % et 150 % restent requises avant l’intégration dans `PdfView.svelte`.
